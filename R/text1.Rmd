...
## Example 2 revisited
Now we are going to use the logisti regresion model to analyse data from example 2. Here we present briefly the data, treatment vs behavior modifications:
...
Or obtained from logisti regresion model \@ref(eq:bmreg) are not the same as thos obtained by means of the function `oddsratio`. The OR's from that functions are these:
...
which are quite similar to those obtained trhoug function `oddsratio`